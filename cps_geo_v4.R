...
mapping <- fips_codes %>%
  mutate(
    county_key  = stri_trans_general(county, "latin-ascii") %>%
      ...
      str_remove_all("\\s(county|parish|borough|census area|municipality|planning region)$") %>%
      str_remove_all("[^a-z0-9]"),
    state_key   = str_remove_all(state_name, "[^A-Za-z0-9]") %>% str_to_lower(),
...
tt <- read_csv(
  "ct_town_county_old_new_codes_with_pop.csv",
  col_types = cols(
    town             = col_character(),
...
ct_county_pop <- tt %>%
  ...
  mutate(GESTFIPS = 9) %>%
  left_join(mapping %>% filter(state_code == 9) %>% select(county_code),
            by = c("GTCO" = "county_code")) %>%
  select(GESTFIPS, GTCO, population)

# 4. Combine cleaned data with FIPS mapping, replace CT
...
cbsa_county <- read_csv("cbsa2fipsxw.csv", col_types = cols(
  cbsacode       = col_integer(),
  fipsstatecode  = col_integer(),
  fipscountycode = col_integer()
)) %>%
  rename(GTCBSA = cbsacode, GESTFIPS = fipsstatecode, GTCO = fipscountycode) %>%
  distinct(GTCBSA, GESTFIPS, GTCO)

# 6. Prepare household flags
hh_min <- read_csv("hhpub24.csv",
                   col_types = cols_only(
...
cbsa_final <- bind_rows(
  cbsa_candidates %>% filter(GESTFIPS != 9),
  cbsa_ct
) %>%
...
# 9. Refined state-level fallback:
#    • Remove all counties tied to ANY reported CBSA
#    • Also remove all counties already present in hhpub24
#    • From remaining, pick the most populous county per state
#    • If none remain, fall back to the state’s single most populous county

# 9A. List of counties tied to reported CBSA
reported_cbsa_counties_all <- cbsa_county %>%
  semi_join(reported_cbsa, by = c("GESTFIPS","GTCBSA")) %>%
  distinct(GESTFIPS, GTCO)

# 9B. List of counties already in hhpub24
reported_counties <- hh_min %>%
  filter(has_county) %>%
  distinct(GESTFIPS, GTCO)

# 9C. All counties in each state
all_state_counties <- pop_fips_final %>%
  distinct(GESTFIPS, GTCO)

# 9D. Candidate counties = all_state_counties
#       minus reported_cbsa_counties_all
#       minus reported_counties
candidate_counties <- all_state_counties %>%
  anti_join(reported_cbsa_counties_all,
            by = c("GESTFIPS","GTCO")) %>%
  anti_join(reported_counties,
            by = c("GESTFIPS","GTCO"))

# 9E. Attach population to candidates
candidate_pop <- candidate_counties %>%
  left_join(pop_fips_final, by = c("GESTFIPS","GTCO")) %>%
  filter(!is.na(population))

# 9F. Pick top-population county among candidates
...
# 9G. States with no candidates → fallback to state’s largest county
...
# 11. Largest towns for CT and New England
ct_town_max <- tt %>%
  group_by(old_county_code) %>%
...
ne_states <- c(23, 25, 33, 44, 50)
ne_town_pop <- read_csv("ne_pop_fips_final.csv", col_types = cols(.default = "c")) %>%
  transmute(GESTFIPS = as.integer(GESTFIPS), GTCO = as.integer(GTCO),
            town_name = cityntown_raw, population = as.numeric(population)) %>%
...
# 12. Fallback official names
global_fips <- fips_codes %>%
  transmute(GESTFIPS = as.integer(state_code), GTCO = as.integer(county_code),
            official_county = county, official_state = state_name)

# 13. Assemble final dataset
full_hh <- read_csv("hhpub24.csv") %>% mutate(row_id = row_number())
final_output <- full_hh %>%
  ...
  mutate(
    ...
    final_location    = case_when(
      GESTFIPS == 9                    ~ town,
      GESTFIPS %in% ne_states            ~ town_name,
      TRUE                             ~ final_county_name
    )
  )

# 14. Write results
write_csv(final_output, "hhpub24_with_imputed_geography_fixed.csv")
